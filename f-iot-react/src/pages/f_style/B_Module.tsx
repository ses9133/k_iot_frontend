...
/*
  * CSS Module
  : 전역적인 CSS 적용이 아닌 컴포넌트 단위로 CSS 를 작성하는 기능 (모듈화)
  - 클래스 이름이 자동으로 로컬 범위(Scope)로 변환되는 방식
    > 각 컴포넌트에 대해 고유한 클래스 이름을 생성 + CSS 스타일 적용

  ex) Button.tsx 컴포넌트가 있다고 하면,
    -> (CSS 모듈 파일명) Button.module.css

  - 클래스 이름 충돌 위험 없음, TypeScript 타입 지원 O 
  - 리액트 컴포넌트 스타일 유지 + 성능/유지보수성/가독성이 좋은 CSS 작성 가능

  1. CSS Module 사용 방법
  : 해당 컴포넌트에서 import 시 CSS 스타일이 객체로 인식됨
  - 내부의 스타일 선택자를 객체의 속성처럼 사용(객체명.클래스명)
...
// * Dashboard 컴포넌트 사용: grid 사용
// cf) 주로 대시보드, 상품 카드 리스트, 갤러리 형태에 사용
//  : 가로, 세로 모두 관리할 때 강력하게 배치 가능

// Grid 레이아웃 시스템
// 1) 설계 방향: 행(row)과 열(column)읭 격자(grid) 기반
