...
const createUserCard = (user: User): HTMLElement => {
  ...
  userCard.dataset.userId = String(user.id);
  // ex) <div class="user-card" data-user-id="3"> 요소 생성됨
...
const addEventListeners = (users: UsersType) => {
  ...
  if(userList) {
    userList.addEventListener('click', (e) => {
      ...
      // - target: 이벤트가 처음 발생한 DOM 요소(실제 클릭된 요소)
      // - currenTarget: 발생한 이벤트가 등록된 DOM 요소(이벤트가 바인딩된 요소, 이벤트 리스너가 걸린 요소)
      const target = e.target as HTMLElement;
      // > 클릭이 발생한 요소는 card 내부의 h2, p 가 될 가능성이 존재

      const userCard = target.closest('.user-card') as HTMLElement;
      // : 이벤트가 발생한 요소의 가장 가까운 .user-card 요소를 반환

      if(userCard) {
        const userId = parseInt(userCard.dataset.userId || '0', 10);
        // dataset: userCard 가 가진 data-* 속성들에 접근할 수 있는 DOM 속성
        // EX) <div data-role="admin" data-userId="1"></div>

        // parseInt(숫자로변환할데이터, 변환진수)
  ...
  if(modal && closeModal) {
    ...
    // cf) document: window 에 로드되는 HTML 문서 그 자체
