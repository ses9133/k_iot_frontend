...
/*
  * Tailwind CSS
  : Utility-First CSS 프레임 워크
  - 미리 디자인된 컴포넌트가 아니라, 원자적 유틸리티 클래스를 조합해 UI 를 만드는 방식

  cf) Bootstrap / MUI처럼 컴포넌트를 제공하는 것이 아니라 CSS 속성이 바로 클래스로 제공
  ...
  4) 디자인 시스템 운영 쉬움
    : 팀 간 UI 스타일 일관성 보장
    : 모든 spacing/color/font 가 scale 기반
  5) 확장성 높음
  ...
  3) 디자인 설계 없이는 똑같은 UI 만 복사`
    : 디자인 시스템 없이 사용하면 tailwind 를 사용한 일반 CSS 와 다름 없음
*/
...
function C_Tailwind() {
  return (
    <div>
      ...
      <div className='text-sm md:text-base lg:text-xl'> 한요소에서 모든 반응형 처리 가능</div>
    </div>
  )
}
