...
container.addEventListener('click', (event) => {
  ...
  console.log('이벤트 리스너가 첨부된 요소 (currentTarget): ' + event.currentTarget);
...
/*
  container 클릭시,
  클릭된 요소 (target): [object HTMLDivElement]
  event02.js:74 이벤트 리스너가 첨부된 요소 (currentTarget): [object HTMLDivElement]
  event02.js:76 이벤트 유형 (type): click

  inner 클릭시, 
  클릭된 요소 (target): [object HTMLButtonElement]
  event02.js:74 이벤트 리스너가 첨부된 요소 (currentTarget): [object HTMLDivElement]
  event02.js:76 이벤트 유형 (type): click
*/

// ! 이벤트의 기본 행동 방지하기
// : form 태그는 submit 버튼 실행시, 내부의 데이터가 서버에 전송 + 페이지가 리로드(새로고침)
// >> preventDefault();
...
form.onsubmit = (e) => {
  ...
  // ? input 요소의 내용값 삽입
  // - placeholder: 힌트 삽입 (작성시 사라짐)
  // >> 참조값(요소값).value속성

  if(fname.value === '' || femail.value === '') {
    // 제출 입력값에 대한 유효성 검증이 모두 이루어지지 않은 경우, 제출 이벤트에 대한 행동 방지
    e.preventDefault(); 
...
//  : e.stopPropagation();
// 이벤트 전파
//  : 자식요소에서 발생한 이벤트가 부모 요소로 전달(이벤트 버블링)
//    이벤트 버블링: 자식 > 부모 > 조상 > document 순으로 올라감!

// +) 이벤트 캡쳐링
//  : 최상위 객체(window or document)에서 이벤트 발생 요소(타깃)로 내려가는 단계
//  >> addEventListener 의 세 번째 인자를 true 로 작성해야, 캡쳐링 단계에서 이벤트 감지 기능(기본값:false - 버블링)
