...
function addTodo() {
  ...
  if (todoInput.value !== '') {
    ...
    //? input 등과 같이 상호작용된 데이터는 사용 후 해당 데이터 초기화
    todoInput.value = '';
  }
}
...
todoInput.addEventListener('keydown', (e) => {
  // ? 특정 키에 반응하여 이벤트 발생시, 이벤트 객체에서 어떤 키의 이벤트인지 확인!
...
todoList.addEventListener('click', (e) => {
  // todoList 내부에 todo-item 이 여러개 나열
  //    : todoItem 클릭시 todo-item 내용의 글자 디자인 변경
  ...
  //  +) DOM 요소의 태그명 가져오기
  //    : HTML요소.tagName
  //    : 태그명의 알파벳은 모두 대문자 표기(A, LI, DIV 등)
  if(e.target.tagName === 'LI') {
    // HTML요소.classList: class 속성에 접근
    
    // cf) .toggle('클래스명') 속성
    //  : classList 에서 해당 클래스명이 토글
    //  - 있으면 삭제, 없으면 추가
