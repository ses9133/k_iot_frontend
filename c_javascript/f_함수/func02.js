...
//  ? 매개변수: 함수에 입력으로 제공되는 데이터(값)
  ...
  // return 결과값; (생략 가능)
  // ? 결과값(return값): 함수가 작업 완료 후 반환하는 데이터(값) - return 키워드로 반환
// }
...
/*
  ...
  -> 익명함수 (function 과 매개변수사이 함수명작성이 안되어있음. const 뒤 변수명으로 대체)
*/
// greet2(); // Cannot access 'greet2' before initialization
console.log('== 함수 표현식 선언 이전 ===');

const greet2 = function() {
  console.log('안녕하세요2, 익명함수는 선언전 호출이 불가능해요');
}

greet2();

// 3) 화살표 함수
// : function 키워드 사용 X - 해당 키워드를 화살표(=>) 로 대체 (간결한 작성), 함수명 작성 X
// : 호이스팅 적용X
// +) 무조건 익명함수로 작성
//    'this' 가 일반 함수가 다르게 바인딩됨
/*
  변수종류 변수명 = (매개변수나열) => {
    함수기능작성
    return 반환값; (선택적)
...
// ? cf) 화살표함수는 매개변수가 1개인 경우 소괄호를, 함수 본문(구현부)이 단일문인 경우 중괄호 + return 키워드 생략 가능
const greet4 = name => console.log(`안녕하세요. ${name} 님`);

// ! 3. 함수의 인자(argument)
// : 함수 사용을 위해 파라미터로 전달되는 값
// - parameter: 변수 선언, argument: 변수 할당
greet4('정은혜');

// ! 4. 함수 호출 방법
// : 함수 생성(선언)
// : 함수 사용(호출) - 함수명(인자값); (인자값은 매개변수가 있을 경우만 작성)
...
function jeh() {
  // 지역변수: 함수 내의 변수(+ 매개변수까지 포함)
  let job = 'developer';
  console.log(`${username}'s job is ${job}`);
}

jeh();
// console.log(job); // ReferenceError: job is not defined >> 지역변수(job)은 해당 스코프를 벗어나면 사용 불가

/*
  함수 선언 방식에 따른 Best Practice
  - 일반적 상황: 선언문(function + 함수명)
